...
This module provides compatibility layer with various pymatgen input sets.
Note that materials project's calculations are done via `custodian` and some parameters may
be changed during the calculation process. The user should check carefully about their compatibility
with calculation run through aiida-vasp.
"""
...
class PymatgenInputAdaptor:
    """
    Generating inputs using pymatgen.io.vasp.sets.
    ...
    def __init__(
        self,
        set_name: str,
        incar_overrides: Optional[Dict] = None,
    ...
    def get_incar_dict(self, structure: orm.StructureData, raw_python: bool = True) -> Union[Dict, orm.Dict]:
        """
    ...
    def get_potential_mapping(self, structure: orm.StructureData) -> Dict[str, str]:
        """
    ...
    def get_potentials(self, structure):
        """
        Return mapping for each symbol to its POTCAR
        """
        return PotcarData.get_potcars_from_structure(
            structure, self.get_potential_family(), self.get_potential_mapping(structure)
        )
    ...
    def get_inputs(self, structure: orm.StructureData, is_workchain=True, overrides=None):
        """Get the inputs for a given structure and overrides"""
        overrides = overrides or {}
        inputs = {}
        incar = self.get_incar_dict(structure, raw_python=True)
        if is_workchain:
            inputs['parameters'] = {'incar': incar}
        else:
